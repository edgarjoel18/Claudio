...
from claudio.context.indexers.factory import get_indexer, get_index_inspector
from claudio.llm.factory import get_llm, get_embedder
from langgraph.checkpoint.sqlite.aio import AsyncSqliteSaver
from claudio.agent.factory import build_agent
from claudio.memory.short_term import get_checkpointer_db_path
from claudio.agent.orchestrator import handle_query
from claudio.memory.session import get_current_session, new_session, switch_session
from claudio.cache.semantic_cache import build_semantic_cache, get_repo_domain
from claudio.observability.logging import get_logger

from claudio.tasks.orchestrator import handle_plan_command
from claudio.tasks.status import show_task_status
from claudio.context.indexers.watcher import start_watcher, stop_watcher
...
async def _run_async():
   logger.info("Starting Claudio")
   console.print("\n[bold blue]Claudio[/bold blue] — RAG-powered code assistant")
       ...
       try:
           while True:
               ...
               if user_input.lower() in ("/exit", "/quit"):
                   ...
                   show_task_status()
               elif len(user_input) == 0 or len(user_input.strip()) == 0:
                   logger.warning(f"Unknown command received: {user_input}")
                   console.print("[yellow]Unknown command. Try:[/yellow]")
                   console.print("  [bold]<question>[/bold]          — ask a question about the codebase")
