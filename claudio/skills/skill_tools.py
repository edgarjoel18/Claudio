...
from claudio.skills.registry import SkillRegistry, SkillNotFoundError
...
def get_registry() -> SkillRegistry:
   ...
   skills_dir resolution order:
     1. Read "skills.skills_dir" from config.yaml  →  e.g. ".claudio/skills"
     2. Resolve relative to Path.cwd()             →  the target project root
        (claudio is invoked from the project being analysed, so cwd()
         IS the target project, not the claudio package itself)


   Example:
     User runs claudio from /home/user/my_project/
     config.yaml says skills_dir: .claudio/skills
     → skills are loaded from /home/user/my_project/.claudio/skills/
   """
   global _registry
   if _registry is None:
       skills_dir = Path.cwd() / config.get("skills", {}).get("skills_dir", ".claudio/skills")
